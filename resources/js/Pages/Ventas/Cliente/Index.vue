      ...
      <template #header>
        <div class="flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            @click="clearFilters"
            class="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition"
          >
            Limpiar filtros
          </button>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <div class="relative">
            ...
            <input
              v-model="search"
              type="text"
              placeholder="Nombre, documento, correo, teléfono..."
              class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#FFEA00] focus:border-transparent transition"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <select
            ...
            class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#FFEA00] focus:border-transparent transition"
          >
            <option value="">Todos</option>
            <option
              v-for="asesor in asesores"
              :key="asesor.id_empleado"
              :value="String(asesor.id_empleado)"
            >
        ...
        <div>
          ...
          >
            ...
            <option
              ...
              :value="String(tipo.id_tipo_cliente)"
            >
              {{ tipo.tipo_cliente }}
            </option>
          </select>
            ...
            <tr v-for="cliente in filtered" :key="cliente.documento" class="hover:bg-gray-50">
              <td class="px-4 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-900">
                  {{ fullAsesorName(cliente.asesor_responsable) }}
                </div>
              </td>
              <td class="px-4 py-4 whitespace-nowrap">
...
const filtered = computed(() => {
  ...
  return props.clientes.filter((c) => {
    ...
    const tipoNombre = (c.tipo_cliente?.tipo_cliente || '').toLowerCase()
    const tipoId = String(c.id_tipo_cliente ?? '')
    const telefono = String(c.telefono ?? '')
    const correo = (c.correo || '').toLowerCase()
    const asesorNombre = fullAsesorName(c.asesor_responsable).toLowerCase()
    const asesorId = String(c.id_empleado_asesor ?? '')
    ...
    const matchesSearch =
      ...
      telefono.includes(q) ||
      correo.includes(q) ||
      asesorNombre.includes(q)

    const matchesAsesor = !selectedAsesor.value || asesorId === selectedAsesor.value
    const matchesTipoCliente = !selectedTipoCliente.value || tipoId === selectedTipoCliente.value

    return matchesSearch && matchesAsesor && matchesTipoCliente
  })
})
...
function handleConfirm() {
  if (clienteToDelete.value) {
    router.delete(`/clientes/${clienteToDelete.value}`)
  }
...
const stats = computed(() => {
  return {
    personasNaturales: filtered.value.filter(
      (c) => c.tipo_cliente?.tipo_cliente === 'Persona Natural'
    ).length,
    personasJuridicas: filtered.value.filter(
      (c) => c.tipo_cliente?.tipo_cliente === 'Persona Jurídica'
    ).length,
    inversionistas: filtered.value.filter((c) => c.tipo_cliente?.tipo_cliente === 'Inversionista')
      .length,
  }
})
...
function fullAsesorName(asesor) {
  if (!asesor) return '—'
  const nombre = asesor.nombre || ''
  const apellido = asesor.apellido || ''
